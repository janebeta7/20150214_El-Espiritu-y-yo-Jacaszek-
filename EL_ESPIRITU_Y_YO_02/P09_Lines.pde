...
class Preset9 extends Preset {
  ...
  private void iniciar1() {
    ...
    for (int i=0; i<num_lineas; i++) {
      ...
      speed[i] =random(2);  // All stripes have a random positive speed
    }
  }
  ...
  void tipo1() {
    int num = int(map(INC, 0, INCMAX, 0, LONG));
    for (int i=0; i<num; i++) {
      //println("-----------------i:"+col[i]);
      ...
      rectMode(CORNER);
      checkBorders(i);
      fill(col[i], o_sound.getLevel()*KNOB_5*10);
      y[i]+=speed[i]-SPEED;
      rect(x[i], y[i]-( o_sound.get_fft(i)*FADER_5), width, (o_sound.get_fft(i))*FADER_5);
    }
  }
  ...
  void tipo2() {
    ...
    for (int i=0; i<num; i++) {
      ...
      fill(col[i], o_sound.getLevel()*KNOB_5*100);
