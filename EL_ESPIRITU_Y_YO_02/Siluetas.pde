...
class Siluetas {
  ...
  public void render() {
    draw(); 
    activadas = BT_100;//kontrol
    isInvert= BT_93; //kontrol
    if (activadas==true) {
      println("------activadas");
      
      displayInvert(0,o_colores.getColor(0));
      if (isPosition ) {
  ...
  void  cargarMask() {
    ...
    if (listado.length > 0) {
      for (int i=0; i<numSiluetas; i++) {
        String nomimg = "siluetas/imagen000"+i+".png";
        String nomimgmask = "siluetas/imagen000"+i+"_a.png";
  ...
  void dibujar() {
...

    {
      if(mousePressed && (TIPO ==1 || TIPO == 2)) {
        ...
        pg.fill(255);
        pg.noStroke();
        if (TIPO == 1 )   pg.ellipse(mouseX,mouseY,RADIO,RADIO);
...
class Formas {
  ...
  void displayInvert(color colorr) {
    ...
    println("Siluetas: ancho"+ancho +"posx:"+posx);
